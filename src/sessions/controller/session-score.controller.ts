...
export class SessionScoreController {
  ...
  @ApiOperation({
    ...
    description:
      'Create or retrieve a score record for a student joining a session',
  })
  @ApiBearerAuth()
  @ApiResponse({
  ...
  @ApiOperation({
    ...
    description:
      'Submit all session results in a single request when the student completes the session. Includes final score, total time, and all answers.',
  })
  @ApiBearerAuth()
  @ApiBody({
  ...
  @ApiOperation({
    ...
    description:
      'Get summary and all scores for a specific session. Only accessible by teachers and admins.',
  })
  @ApiBearerAuth()
  @ApiParam({
    name: 'sessionId',
  ...
  @ApiOperation({
    ...
    description:
      'Get the score record for the authenticated student in a specific session',
  })
  @ApiBearerAuth()
  @ApiParam({
    name: 'sessionId',
  ...
  async getStudentScore(
    ...
    if (!userId && !userCorreo) {
      throw new BadRequestException(
        'User ID or email is required to retrieve score',
      );
    }
  ...
  @ApiOperation({
    ...
    description:
      'Get all score records for the authenticated user across all sessions. Not available for guest users.',
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: HttpStatus.OK,
  ...
  async getUserScores(
    ...
    if (user.isGuest) {
      throw new BadRequestException(
        'Guest users cannot access historical scores. Please register for full access.',
      );
    }
  ...
  @ApiOperation({
    ...
    description:
      'Get all score records for a specific user. Only accessible by teachers and admins.',
  })
  @ApiBearerAuth()
  @ApiParam({
    name: 'userId',
  ...
  @ApiOperation({
    ...
    description:
      'Download all session scores as an Excel file. Only accessible by teachers and admins.',
  })
  @ApiBearerAuth()
  @ApiParam({
    name: 'sessionId',
  ...
  async exportSessionScores(
    ...
    const buffer =
      await this.sessionScoreService.exportSessionScoresToExcel(sessionId);
